...
echo 'select id,gid,groups.gidnumber,memberuidnumber from groups, groups_users WHERE groups_users.gidnumber=groups.gidnumber;' | psql -U postgres -d ldap | grep -v "\-\-+\-\-" | grep -v "rows)" | grep -v "Zeilen)" | grep -v ^$ > /tmp/groups_users.sql
